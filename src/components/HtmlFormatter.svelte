...
<div class="w-1/2 px-2 h-screen pb-24 relative">
  <div class="bg-gray-800 p-6 rounded overflow-auto max-h-full">
    <div class="absolute justify-center items-center right-0 flex mr-10">
      <div
        class="absolute {copied ? 'z-10' : ''} {copied
          ? ''
          : 'opacity-0'} transform duration-1000 {copied
          ? '-translate-y-6'
      ...
      <div
        class="{copied ? '' : 'z-10'} {empty ? 'hidden' : ''} {copied
          ? ''
          : 'cursor-pointer'}"
    ...
    <pre class="">
        <code class="bg-gray-800 text-white" use:SetPre
        >{@html hljs.highlight(b_str, {
          language: "xml",
          ignoreIllegals: true,
        }).value}</code
      >  
